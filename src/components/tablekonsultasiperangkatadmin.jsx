...
const TableKonsultasiPerangkatAdmin = () => {
  ...
  const deletePerbaikan = async (id) => {
    ...
    if (shouldDelete) {
      try {
        await axios.delete(`http://localhost:8080/delete-konsultasiperangkat/${id}`);
        getKonsultasiPerangkat();
      } catch (error) {
          ...
          {dataKonsultasiPerangkat.map((item, index) => (
            <tr key={item.id_konsultasi} onClick={() => viewDetail(item.id_konsultasi)}>
              ...
              <a href={`/updatekonsultasi/${item.id_konsultasi}`} className='btn btn-primary me-md-2'>
                ...
                <button on onClick={() => deletePerbaikan(item.id_konsultasi)} className='btn btn-primary me-md-2'>
                  delete
                </button>
              </td>
            </tr>
...
export default TableKonsultasiPerangkatAdmin;
